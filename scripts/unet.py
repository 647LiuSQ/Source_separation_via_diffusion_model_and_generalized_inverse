...
class ConvNextBlock_(nn.Module):
    """
    https://arxiv.org/abs/2201.03545
    """
    ...
    def __init__(self, dim, dim_out, *, time_emb_dim=None, mult=2, norm=True):
        ...
        self.ds_conv = nn.Conv2d(dim, dim, 7, padding="same", groups=dim)

        self.net = nn.Sequential(
            nn.GroupNorm(1, dim) if norm else nn.Identity(),
            nn.Conv2d(dim, dim_out * mult, 3, padding="same"),
            nn.GELU(),
            nn.GroupNorm(1, dim_out * mult),
            nn.Conv2d(dim_out * mult, dim_out, 3, padding="same"),
        )
...
class Unet2(nn.Module):
    ...
    def forward(self, x, time):
        ...
        # downsample
        for block1, block2,block3,block4,block5,block6,block7,block8, attn, downsample in self.downs:
            x = block1(x, t)
            x = block2(x, t)
            x = block3(x, t)
            x = block4(x, t)
        ...
        # upsample
        for block1, block2,block3,block4,block5,block6,block7,block8, attn, upsample in self.ups:
            x = torch.cat((x, h.pop()), dim=1)
            x = block1(x, t)
            x = block2(x, t)
            x = block3(x, t)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        block_klass(dim_in, dim_out, time_emb_dim=time_dim),
                        Residual(PreNorm(dim_out, Attention(dim_out))) if (ind==1)or(ind==2) else nn.Identity(),
                        block_klass(dim_out, dim_out, time_emb_dim=time_dim),
                        Residual(PreNorm(dim_out, Attention(dim_out))) if (ind==1)or(ind==2) else nn.Identity(),
                        Downsample(dim_out) if not is_last else nn.Identity(),
                    ]
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        block_klass(dim_out * 2, dim_in,
                                    time_emb_dim=time_dim),
                        Residual(PreNorm(dim_in, Attention(dim_in))) if (ind == 1) or (ind == 2) else nn.Identity(),
...
class Unet5(nn.Module):
    def __init__(
        ...
        init_dim = default(init_dim, dim // 3 * 2)
        self.init_conv = nn.Conv2d(channels, init_dim, 7, padding="same")

        dims = [init_dim, *map(lambda m: dim * m, dim_mults)]
        in_out = list(zip(dims[:-1], dims[1:]))

        block_klass = partial(ConvNextBlock_, mult=convnext_mult)
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        ...
                        block_klass(dim_out, dim_out, time_emb_dim=time_dim),
                        block_klass(dim_out, dim_out, time_emb_dim=time_dim),
                        Downsample_() if not is_last else nn.Identity(),
                    ]
                )
            )
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        ...
                        block_klass(dim_in + dim_out, dim_in, time_emb_dim=time_dim),
                        block_klass(dim_in + dim_out, dim_in, time_emb_dim=time_dim),
                        Upsample_() if not is_last else nn.Identity(),
                    ]
                )
            )
